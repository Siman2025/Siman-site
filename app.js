...
save(state); // ensure initialized

// Admin toggle
let adminOn = false;
$('#adminToggle').addEventListener('click', ()=>{
  adminOn = !adminOn;
  $('#adminToggle').textContent = adminOn ? 'Admin: On' : 'Admin: Off';
...
function renderBookingsTable(){
  ...
  state.bookings.forEach(b=>{
    ...
    tr.innerHTML = `<td>${b.name}</td><td class="muted small">${svc}</td><td>${when}</td><td class="muted small">${b.email}<br/>${b.suburb}</td><td>${b.notes||''}</td><td><button class="btn small" data-id="${b.id}">Cancel</button></td>`;
